"""Field-level mismatches, classified by the shape of the error.

:func:`analyze_prediction_errors` lists every gold field the prediction got wrong and
labels each with *why* it is wrong -- a formatting difference, a boolean written the
other way, a value invented where gold is blank -- so systematic failures separate
from one-off ones.  :func:`create_error_report` groups those rows into the frequency
table the paper reports.

The classification is diagnostic only: which fields count as wrong is decided by
:mod:`analysis.metrics.matching`, the same rule the accuracy and precision/recall
tables use, so no error type can appear for a field those tables score as correct.
"""
...
from analysis.metrics import _is_missing, _values_match
...
_REPORT_COLUMNS = ["error_type", "field_type", "expected_value", "predicted_value", "frequency", "assays"]
...
def analyze_prediction_errors(
    ...
    *,
    ...
    return pd.DataFrame(rows, columns=_ERROR_COLUMNS)
