...
def _get_cache_dir() -> Path:
    """Get the platform-appropriate cache directory for arms-agent.

    Uses the ``ARMS_CACHE_DIR`` environment variable if set,
    otherwise falls back to a platform-specific default.
    """
    env_override = os.environ.get("ARMS_CACHE_DIR")
    if env_override:
        return Path(env_override)

    system = platform.system()
    if system == "Darwin":
        return Path.home() / "Library" / "Caches" / "arms-agent"
    elif system == "Windows":
        local_app_data = os.environ.get("LOCALAPPDATA", "")
        if local_app_data:
            return Path(local_app_data) / "arms-agent" / "cache"
        return Path.home() / "AppData" / "Local" / "arms-agent" / "cache"
    else:
        xdg_cache = os.environ.get("XDG_CACHE_HOME", "")
        if xdg_cache:
            return Path(xdg_cache) / "arms-agent"
        return Path.home() / ".cache" / "arms-agent"


def _get_ttl() -> int:
    """Get the TTL for cache entries in seconds.

    Reads from ``ARMS_CACHE_TTL_SECONDS``, falling back to 86400 (24 hours).
    """
    env_val = os.environ.get("ARMS_CACHE_TTL_SECONDS")
    ...
    def __init__(
        ...
        Args:
            ...
            ttl_seconds: TTL for cache entries. Defaults to the value from
                        ``ARMS_CACHE_TTL_SECONDS`` or 86400.
        """
        if db_path is None:
